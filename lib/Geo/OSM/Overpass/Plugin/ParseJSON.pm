package Geo::OSM::Overpass::Plugin::ParseJSON;
...
# the name of the file containing JSON
# returns undef on failure
# WARNING: instead of 'nodes' as with XML, we get 'elements' for JSON
sub gorun {
	my $self = $_[0];
	my $params = $_[1];
	...
	#}
	my $conv = JSON->new();
	if( ! defined $conv ){ print STDERR "$whoami (via $parent) : call to ".'JSON->new()'." has failed.\n"; return undef }

	my $res = undef;
	if( exists $params->{'input-string'} ){
		if( 'SCALAR' eq ref $params->{'input-string'} ){
			$res = $conv->decode(${$params->{'input-string'}});
		} else { 
			$res = $conv->decode($params->{'input-string'});
		}
		...
		$res = $conv->decode($content);
	} else {
		my $eng = $self->engine();
		if(  ! defined $eng ){ print STDERR "$whoami (via $parent) : engine is undefined therefore only supplied filename or JSON strings can be processed.\n"; return undef }
		my $resref = $eng->last_query_result();
		if( ! defined $resref ){ print STDERR "$whoami (via $parent) : last query result is undefined, there is no JSON to process either from the input params ('input-string' or 'input-filename' or from the last query result).\n"; return undef }
		$res = $conv->decode($$resref);
	}
	if( ! defined $res ){ print STDERR "$whoami (via $parent) : call to ".'decode()'." has failed.\n"; return undef }
	...
	#	       'timestamp_osm_base' => '2019-05-11T12:53:02Z'
	#    }
...
Geo::OSM::Overpass::Plugin::ParseJSON - Plugin for L<Geo::OSM::Overpass> to fetch bus stop data in given area
...
its C<gorun()> method. (note: no bounding box is required)
    ...
    my $perlstruct = $plug->gorun({'input-string' => '{"abc" : "xyz"}') or die;
...
It will convert OSM JSON results to a Perl hashtable. The JSON to
...
Please report any bugs or feature requests to C<bug-geo-osm-overpass-plugin-ParseJSON at rt.cpan.org>, or through
the web interface at L<https://rt.cpan.org/NoAuth/ReportBug.html?Queue=Geo-OSM-Overpass-Plugin-ParseJSON>.  I will be notified, and then you'll
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc Geo::OSM::Overpass::Plugin::ParseJSON
...
L<https://rt.cpan.org/NoAuth/Bugs.html?Dist=Geo-OSM-Overpass-Plugin-ParseJSON>

=item * AnnoCPAN: Annotated CPAN documentation

L<http://annocpan.org/dist/Geo-OSM-Overpass-Plugin-ParseJSON>

=item * CPAN Ratings

L<https://cpanratings.perl.org/d/Geo-OSM-Overpass-Plugin-ParseJSON>

=item * Search CPAN

L<https://metacpan.org/release/Geo-OSM-Overpass-Plugin-ParseJSON>
...
1; # End of Geo::OSM::Overpass::Plugin::ParseJSON
